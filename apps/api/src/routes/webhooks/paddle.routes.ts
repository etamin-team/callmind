...
const paddleRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (!rawBody) {
        ...
        return reply
          .status(400)
          .send({ error: "Raw body required for verification" });
      }
      ...
      try {
        ...
        try {
          ...
          fastify.log.warn(
            { err: sigError },
            "Primary unmarshal failed, trying alternative",
          );
          // For newer SDK versions, unmarshal might need all 3 args
          // Try with dummy secret if needed, or parse manually
  ...
  async function handleTransactionCompleted(transaction: any) {
    ...
    if (!userId && !customerEmail) {
      fastify.log.warn(
        ...
        "No userId or email found in transaction",
      );
      return;
    }
    ...
    if (creditsToAdd > 0) {
      ...
      await user.save();
      fastify.log.info(
        { userId: user.id, creditsAdded: creditsToAdd },
        "Credits granted from transaction",
      );
    }
  }
  ...
  async function handleSubscriptionCreated(subscription: any) {
    ...
    if (!userId && !customerEmail) {
      fastify.log.warn(
        ...
        "No userId or email found in subscription",
      );
      return;
    }
    ...
    await user.save();
    fastify.log.info(
      { userId: user.id, plan: user.plan, subscriptionId: subscription.id },
  ...
  async function handleSubscriptionUpdated(subscription: any) {
    ...
    if (!user) {
      ...
      if (
        ...
        userByEmail.plan = "enterprise";
      }

      await userByEmail.save();
      fastify.log.info(
        { userId: userByEmail.id, plan: userByEmail.plan },
        "Subscription updated (found by email)",
      );
      return;
    }
  ...
  async function handleSubscriptionCanceled(subscription: any) {
    ...
    fastify.log.info(
      { userId: user.id },
      "Subscription canceled, user downgraded to free plan",
    );
  }
};
